...
const EditEvent = () => {
  ...
  // Fetch organizers based on search query
  const fetchOrganizerSearchResults = async (query) => {
    ...
    try {
      ...
      setOrganizerSearchResults(response.data.users || []);
    } catch (error) {
    ...
    const fetchEvent = async () => {
      try {
        ...
        setFormData({
          ...
          organizers: event.organizers.map((id) => ({ id, name: "" })), // Names fetched separately if populated
          ...
          <div className="space-y-3">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-2">
              <input
                ...
                onChange={(e) =>
                  setNewChiefGuest((prev) => ({ ...prev, name: e.target.value }))
                }
                  ...
                  <label className="flex items-center justify-center w-full h-10 cursor-pointer border border-dashed border-gray-300 rounded-lg hover:border-indigo-500 transition-colors text-sm">
                    ...
                    <input
                      ...
                      onChange={(e) => {
                        ...
                        if (file) {
                          handleFileUpload(file, (url) =>
                            setNewChiefGuest((prev) => ({ ...prev, picture: url }))
                          );
                        }
                      }}
                      disabled={uploading}
      ...
      {showModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-xl p-6 max-w-md w-full">
            <h3 className="text-xl font-semibold mb-4">Confirm Event Update</h3>
